...
export function Scoreboard({
  ...
  const sortedSongsFull = useMemo(() => {
    return Object.entries(eloRatings)
      ...
      .filter(Boolean)
      .sort((a, b) => (b?.elo ?? INITIAL_ELO) - (a?.elo ?? INITIAL_ELO)) as Song[];
  }, [songs, eloRatings]);
    ...
    <section class={styles.scoreboard}>
      <div class={styles.scoreboardHeader}>
        ...
        <div class={styles.headerButtons}>
          {linkToOwnBoard && currentTheme && (
            <button
              class={styles.button}
              onClick={() => (window.location.href = `/${currentTheme.id}`)}
            >
              Your Scoreboard
            </button>
          )}
      ...
      {sortedSongsFull.length > SCOREBOARD_INITIAL_LIMIT && (
        <button class={styles.toggleScoreboardButton} onClick={toggleShowAll}>
          {showAll
            ? `Show Top ${SCOREBOARD_INITIAL_LIMIT}`
            : `Show All ${sortedSongsFull.length} Results`}
        </button>
      )}
    </section>
  );
